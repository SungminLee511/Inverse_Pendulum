// Generated by tools/derive_eom.py for n=2 — DO NOT EDIT BY HAND.
// Equations of motion for the inverted 2-link pendulum on a cart.
//
// Sign convention: angles theta_i measured from the upward vertical, CCW positive.
// q     = [x, theta_1, ..., theta_2]  (length 3)
// qdot  = [xdot, theta_1_dot, ..., theta_2_dot]
...
export function M(q, qdot, params) {
  ...
  return [
    [m0 + m1 + m2, (L1*m2 + lc1*m1)*Math.cos(th1), lc2*m2*Math.cos(th2)],
    [(L1*m2 + lc1*m1)*Math.cos(th1), I1 + Math.pow(L1, 2)*m2 + Math.pow(lc1, 2)*m1, L1*lc2*m2*Math.cos(th1 - th2)],
    [lc2*m2*Math.cos(th2), L1*lc2*m2*Math.cos(th1 - th2), I2 + Math.pow(lc2, 2)*m2]
  ];
}
...
export function Cqdot(q, qdot, params) {
  ...
  const m2 = params.links[1].m, L2 = params.links[1].L, lc2 = params.links[1].l, I2 = params.links[1].I, cv2 = params.links[1].joint_viscous;
  return [
    -L1*m2*Math.pow(thd1, 2)*Math.sin(th1) - lc1*m1*Math.pow(thd1, 2)*Math.sin(th1) - lc2*m2*Math.pow(thd2, 2)*Math.sin(th2),
    L1*lc2*m2*Math.pow(thd2, 2)*Math.sin(th1 - th2),
    -L1*lc2*m2*Math.pow(thd1, 2)*Math.sin(th1 - th2)
  ];
}
...
export function G(q, qdot, params) {
  ...
  return [
    0,
    -g*(L1*m2 + lc1*m1)*Math.sin(th1),
    -g*lc2*m2*Math.sin(th2)
  ];
}
